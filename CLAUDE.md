...
- SQS: AI 系 REST エンドポイント（生成/URL生成/採点/補足/アドバイス/チューター）を非同期ジョブ化（submit→202+job_id→`GET /ai-jobs/{id}` ポーリング。API Gateway 30 秒上限の恒久対応。ローカルは `AI_JOB_WORKER_MODE=inline`。設計: `docs/design/ai-async-jobs/`）
