...
class CardService:
    ...
    def bulk_create_cards(
        ...
        """複数のカードデータをまとめて作成し、成功件数を返す。

        URL カード生成の保存フロー（ref-key 経由 / レガシー）で共通利用する。
        各カードの作成失敗は握りつぶしてログに記録し、残りの保存を継続する
        （部分成功を許容）。``front`` / ``back`` が空のカードはスキップする。

        M-19: 全件失敗時は 0 を返す。呼び出し側は「0 = 保存失敗」として扱い、
        誤って成功扱いの通知を出さないこと。

        Args:
            user_id: ユーザー ID。
            cards: カードデータ dict のリスト。各 dict は ``front`` / ``back`` を
                持ち、タグは ``suggested_tags`` または ``tags`` キーで指定する（任意）。
            references: 全カードに共通で付与する参照（URL 等）。
        ...
        for card in cards:
            ...
            try:
                ...
                logger.warning(
                    "Failed to save card in bulk_create_cards",
                    extra={"error": str(e)},
                )
        return saved_count
