...
- 招待制サインアップ（prod。PreSignUp トリガー + 許可リストで新規登録を制限）
...
# prod は必須: サインアップ許可リストの Invoke 許可に使う Cognito UserPool の ARN
export COGNITO_USER_POOL_ARN="arn:aws:cognito-idp:ap-northeast-1:<ACCOUNT_ID>:userpool/<UserPoolId>"
...
- `COGNITO_USER_POOL_ARN`（**prod 必須**） - サインアップ許可リストの PreSignupFunction に対する Invoke 許可（`SourceArn`）に使う Cognito UserPool の ARN（Cognito Stack Output `UserPoolArn`）
...
prod は招待制。Cognito PreSignUp トリガーが DynamoDB の許可リストと照合し、未許可のサインアップを拒否する。

- **メール**: 招待時に運用者が事前にメールアドレスを許可リストへ登録しておく。
- **LINE**: `sub` を事前に知る手段がないため、本人が一度ログインを試行 → 拒否されて `pending` として記録 → 運用者が承認 → 本人が再ログイン、という流れになる。

```bash
cd backend
make allowlist-add     ENV=prod ID=friend@example.com NOTE="..."       # 即 approved で登録
make allowlist-list    ENV=prod                                        # 全件表示
make allowlist-approve ENV=prod ID='idp#line_u1234...' NOTE="..."      # pending → approved
make allowlist-remove  ENV=prod ID=friend@example.com                  # 削除（新規登録のみ防止。既存アカウントは残る）
make verify-presignup  ENV=prod COGNITO_USER_POOL_ARN=<UserPoolArn>    # PreSignUp トリガー配線の検証
```

スイッチロール済みの `AWS_PROFILE` を前提とする。identifier の正規化規則・LINE 承認時の本人確認手順・デプロイ手順は [signup-allowlist アーキテクチャ設計](docs/design/signup-allowlist/architecture.md) を参照。
