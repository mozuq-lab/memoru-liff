...
## PR #86 レビュー指摘（オーナーレビュー、修正済み）

### P1: 許可判定が結果整合性読み取り

- **場所**: `backend/src/services/allowlist_service.py` (get_status)
- **問題**: `GetItem` は既定で結果整合性のため、`allowlist-remove` 直後に古い approved が返り、
  削除済み識別子の新規登録を許可しうる
- **対応**: ✅ `ConsistentRead=True` を指定（アクセス制御判定のため強整合性）。引数を検証する
  回帰テストを追加

### P2: Makefile が ID / NOTE をレシピ文字列へテキスト展開

- **場所**: `backend/Makefile` (allowlist-add / approve / remove ほか)
- **問題**: `ALLOWLIST_NOTE="$(NOTE)"` は Make が先にテキスト置換するため、値中のダブルクォート
  が失われ（`NOTE='友人 "A"'` → `友人 A`）、値に含まれる `$( )` はコマンド置換として実行される
  （前回修正 #2 はシェル層のクォートのみで Make 層の展開が残っていた）
- **対応**: ✅ `export ENV ID NOTE COGNITO_USER_POOL_ARN` で環境変数としてレシピへ渡し、
  Make のテキスト展開を全廃（正規化・タイムスタンプ・NOTE 有無分岐は python3 側で
  `os.environ` から読む。python コード文字列に `$` を含めない）。fake-aws シムによる実測で
  ダブルクォート保持・コマンド置換不実行・trim/小文字化を確認（Sonnet 修正 + Fable 独立再検証）

### P2 続報: export 時の Make 展開で単一の `$` が消失（再レビュー指摘）

- **場所**: `backend/Makefile`（前回 P2 修正の `export ENV ID NOTE ...`）
- **問題**: コマンドライン変数は再帰展開変数のため、export の環境構築時に Make が値を再展開し
  単一の `$` が消費される（`NOTE='cost $5'` → `cost `、`ID='user$tag@...'` → `userag@...`）。
  `$` は RFC 5322 atext に含まれメール local-part で合法
- **対応**: ✅ `$(value VAR)` で未展開の生値を即時変数（`ALLOWLIST_RAW_*`）に固定してから
  export する方式に変更（`:=` の代入・export とも再展開しないことを単体 .mk で実測確認のうえ
  採用）。fake-aws シムを使って make を subprocess 実行する回帰テスト 5 件を
  `tests/integration/test_makefile_allowlist.py` に追加（修正前 Makefile で 2 件失敗する
  ことを確認済み）。環境変数渡し（`NOTE='...' make ...`）でも動作することを実測確認
