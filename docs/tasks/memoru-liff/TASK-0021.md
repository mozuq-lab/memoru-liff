# TASK-0021: LINE統合テスト

**タスクID**: TASK-0021
**タスクタイプ**: TDD
**推定工数**: 8時間
**フェーズ**: Phase 4 - 統合テスト
**信頼性レベル**: 🟡 *acceptance-criteria.mdより妥当な推測*

## 関連文書
- **概要**: [overview.md](overview.md)
- **要件定義**: [requirements.md](../../spec/memoru-liff/requirements.md) - REQ-041, REQ-051, REQ-412
- **設計文書**: [architecture.md](../../design/memoru-liff/architecture.md)
- **受け入れ基準**: [acceptance-criteria.md](../../spec/memoru-liff/acceptance-criteria.md)

## タスク概要
LINE Messaging APIとの統合テストを実装する。Webhook受信処理、Flex Message送信、Postback処理、定期復習通知の各機能について、モックを活用した統合テストスイートを構築する。

## 依存タスク
- **前提タスク**: TASK-0010（LINE Messaging API統合）, TASK-0011（Webhook受信ハンドラ）
- **後続タスク**: TASK-0022（本番デプロイ準備）

## 完了条件
- [ ] LINE Webhook統合テスト実装
- [ ] Flex Message送信テスト実装
- [ ] Postback処理テスト実装
- [ ] 定期通知テスト実装
- [ ] LINE APIモック設定完了
- [ ] すべてのテストがパス

---

## 実装詳細

### 1. LINE APIモック設定 🟡
**信頼性**: 🟡 *一般的なモックパターンから推測*

LINE Messaging APIのモッククライアントを設定する。

```python
# backend/tests/conftest.py
import pytest
from unittest.mock import MagicMock, patch

@pytest.fixture
def mock_line_client():
    """LINE Messaging APIクライアントのモック"""
    with patch('linebot.v3.messaging.MessagingApi') as mock:
        client = MagicMock()
        mock.return_value = client

        # push_message のモック
        client.push_message.return_value = None

        # reply_message のモック
        client.reply_message.return_value = None

        yield client

@pytest.fixture
def webhook_signature():
    """Webhook署名生成ヘルパー"""
    import hmac
    import hashlib
    import base64

    def generate(body: str, channel_secret: str) -> str:
        hash = hmac.new(
            channel_secret.encode('utf-8'),
            body.encode('utf-8'),
            hashlib.sha256
        ).digest()
        return base64.b64encode(hash).decode('utf-8')

    return generate
```

**実装ファイル**: `backend/tests/conftest.py`

### 2. Webhook統合テスト 🟡
**信頼性**: 🟡 *acceptance-criteria.mdより推測*

LINE WebhookイベントのE2E処理をテストする。

```python
# backend/tests/integration/test_line_webhook.py
import pytest
import json
from fastapi.testclient import TestClient
from app.main import app

class TestLineWebhook:
    """LINE Webhook統合テスト"""

    def test_postback_grade_5_updates_card(
        self,
        client: TestClient,
        mock_line_client,
        webhook_signature,
        test_card
    ):
        """TC-032-01: grade=5でのPostback処理テスト"""
        # Arrange
        postback_data = f"action=grade&card_id={test_card.id}&grade=5"
        event = {
            "events": [{
                "type": "postback",
                "replyToken": "test_reply_token",
                "source": {"userId": "U1234567890", "type": "user"},
                "postback": {"data": postback_data}
            }]
        }
        body = json.dumps(event)
        signature = webhook_signature(body, "test_channel_secret")

        # Act
        response = client.post(
            "/webhook/line",
            content=body,
            headers={
                "X-Line-Signature": signature,
                "Content-Type": "application/json"
            }
        )

        # Assert
        assert response.status_code == 200

        # カード情報が更新されていることを確認
        updated_card = get_card_by_id(test_card.id)
        assert updated_card.ease_factor > test_card.ease_factor
        assert updated_card.review_count == test_card.review_count + 1

        # 返信メッセージが送信されたことを確認
        mock_line_client.reply_message.assert_called_once()

    def test_webhook_signature_validation(
        self,
        client: TestClient
    ):
        """不正な署名のWebhookは拒否される"""
        event = {"events": []}
        body = json.dumps(event)

        response = client.post(
            "/webhook/line",
            content=body,
            headers={
                "X-Line-Signature": "invalid_signature",
                "Content-Type": "application/json"
            }
        )

        assert response.status_code == 400
```

**実装ファイル**: `backend/tests/integration/test_line_webhook.py`

### 3. 定期通知統合テスト 🟡
**信頼性**: 🟡 *acceptance-criteria.mdより推測*

復習通知のプッシュ送信をテストする。

```python
# backend/tests/integration/test_due_push.py
import pytest
from datetime import datetime, timedelta
from unittest.mock import MagicMock, call

class TestDuePushNotification:
    """定期復習通知統合テスト"""

    def test_tc_041_01_single_due_card_notification(
        self,
        mock_line_client,
        test_user,
        due_card
    ):
        """TC-041-01: 復習期限カードの通知送信"""
        # Arrange
        from app.services.notification import NotificationService
        service = NotificationService(line_client=mock_line_client)

        # Act
        result = service.send_due_notifications()

        # Assert
        mock_line_client.push_message.assert_called_once()
        call_args = mock_line_client.push_message.call_args

        # ユーザーIDが正しいこと
        assert call_args.kwargs['to'] == test_user.line_user_id

        # Flex Messageが送信されていること
        message = call_args.kwargs['messages'][0]
        assert message.type == 'flex'
        assert '復習カード' in str(message.contents)

    def test_tc_041_02_multiple_due_cards_notification(
        self,
        mock_line_client,
        test_user,
        multiple_due_cards
    ):
        """TC-041-02: 複数カードの通知テスト"""
        # Arrange
        from app.services.notification import NotificationService
        service = NotificationService(line_client=mock_line_client)

        # Act
        result = service.send_due_notifications()

        # Assert
        mock_line_client.push_message.assert_called_once()
        call_args = mock_line_client.push_message.call_args

        # Flex Messageに複数カードが含まれること
        message = call_args.kwargs['messages'][0]
        assert message.type == 'flex'

        # Carousel形式で複数カードが表示されること
        contents = message.contents
        assert contents.type == 'carousel'
        assert len(contents.contents) == len(multiple_due_cards)

    def test_no_due_cards_no_notification(
        self,
        mock_line_client,
        test_user
    ):
        """復習期限カードがない場合は通知しない"""
        # Arrange
        from app.services.notification import NotificationService
        service = NotificationService(line_client=mock_line_client)

        # Act
        result = service.send_due_notifications()

        # Assert
        mock_line_client.push_message.assert_not_called()
```

**実装ファイル**: `backend/tests/integration/test_due_push.py`

### 4. Flex Message構築テスト 🟡
**信頼性**: 🟡 *LINE Messaging API仕様から推測*

Flex Messageの構造と内容をテストする。

```python
# backend/tests/integration/test_flex_message.py
import pytest
from app.services.line_message import FlexMessageBuilder

class TestFlexMessageBuilder:
    """Flex Message構築テスト"""

    def test_build_review_card_message(self, test_card):
        """復習カードのFlex Message構築"""
        builder = FlexMessageBuilder()
        message = builder.build_review_card(test_card)

        assert message.type == 'flex'
        assert message.alt_text == '復習カードがあります'

        # Bubble構造の確認
        bubble = message.contents
        assert bubble.type == 'bubble'
        assert bubble.body is not None
        assert bubble.footer is not None

        # Postbackアクションの確認
        footer = bubble.footer
        buttons = footer.contents
        assert len(buttons) == 5  # grade 1-5

        for i, button in enumerate(buttons, 1):
            assert button.action.type == 'postback'
            assert f'grade={i}' in button.action.data

    def test_build_carousel_message(self, multiple_cards):
        """複数カードのCarousel Message構築"""
        builder = FlexMessageBuilder()
        message = builder.build_review_carousel(multiple_cards)

        carousel = message.contents
        assert carousel.type == 'carousel'
        assert len(carousel.contents) == len(multiple_cards)
```

**実装ファイル**: `backend/tests/integration/test_flex_message.py`

---

## テスト要件

### テストケース1: 復習通知送信 🟡
**信頼性**: 🟡 *acceptance-criteria.mdより推測*
**関連TC**: TC-041-01
**テスト内容**: 復習期限が到来したカードについて、該当ユーザーにLINEプッシュ通知が送信されることを確認する。

### テストケース2: 複数カードの通知 🟡
**信頼性**: 🟡 *acceptance-criteria.mdより推測*
**関連TC**: TC-041-02
**テスト内容**: 複数の復習期限カードがある場合、Carousel形式のFlex Messageで一括通知されることを確認する。

### テストケース3: grade=5での採点処理 🟡
**信頼性**: 🟡 *acceptance-criteria.mdより推測*
**関連TC**: TC-032-01
**テスト内容**: PostbackイベントでGrade 5が選択された場合、SM-2アルゴリズムに従ってカード情報が更新され、次回復習日が設定されることを確認する。

---

## 実装手順

### TDDタスクの場合
1. `/tsumiki:tdd-requirements TASK-0021` - 詳細要件定義
2. `/tsumiki:tdd-testcases` - テストケース作成
3. `/tsumiki:tdd-red` - テスト実装（失敗）
4. `/tsumiki:tdd-green` - 最小実装
5. `/tsumiki:tdd-refactor` - リファクタリング
6. `/tsumiki:tdd-verify-complete` - 品質確認

---

## 注意事項
- LINE Messaging APIへの実際のリクエストは行わず、すべてモックを使用すること
- Webhook署名検証は本番と同じロジックを使用し、テスト用の署名を生成すること
- 定期通知テストはEventBridge Schedulerのトリガーではなく、サービス層の直接呼び出しでテストする
- テストデータのセットアップにはfixtureを活用し、テスト間の独立性を保つ

---

## 信頼性レベルサマリー
- 🔵 **青信号**: 0項目
- 🟡 **黄信号**: 7項目（APIモック、Webhookテスト、定期通知テスト、Flex Messageテスト、TC-041-01〜02、TC-032-01）
- 🔴 **赤信号**: 0項目

**品質評価**: 要改善 - acceptance-criteria.mdに基づく推測が主であり、LINE API仕様との整合性確認が必要
