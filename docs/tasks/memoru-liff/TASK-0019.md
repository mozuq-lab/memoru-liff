# TASK-0019: LINE連携画面

**タスクID**: TASK-0019
**タスクタイプ**: TDD
**推定工数**: 4時間
**フェーズ**: Phase 3 - フロントエンド実装
**信頼性レベル**: 🔵 *user-stories.md 1.2より*

## 関連文書
- **概要**: [overview.md](overview.md)
- **要件定義**: [requirements.md](../../spec/memoru-liff/requirements.md) - REQ-003, REQ-202
- **設計文書**: [architecture.md](../../design/memoru-liff/architecture.md)

## タスク概要
LINE連携の状態表示と連携処理を行う画面を実装する。LIFF SDK getProfile()を使用してLINEユーザー情報を取得し、サーバーと連携する。

## 依存タスク
- **前提タスク**: TASK-0006（ユーザー設定API）, TASK-0014（ホーム画面・ナビゲーション）
- **後続タスク**: なし

## 完了条件
- [x] LINE連携状態表示実装
- [x] LIFF SDK getProfile()呼び出し実装
- [x] 連携APIリクエスト実装
- [x] 単体テスト80%以上のカバレッジ達成

---

## 実装詳細

### 1. LINE連携画面 🔵
**信頼性**: 🔵 *user-stories.md 1.2より*

LINE連携の状態表示と連携処理を行う画面を実装する。

```typescript
// frontend/src/pages/LinkLinePage.tsx
import { useState, useEffect, useCallback } from 'react';
import { useNavigate } from 'react-router-dom';
import { Loading } from '../components/common/Loading';
import { Error } from '../components/common/Error';
import { usersApi } from '../services/api';
import { getLiffProfile, initializeLiff } from '../services/liff';

interface LineStatus {
  isLinked: boolean;
  lineUserId?: string;
  displayName?: string;
  pictureUrl?: string;
}

export const LinkLinePage = () => {
  const navigate = useNavigate();
  const [lineStatus, setLineStatus] = useState<LineStatus | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  const [isLinking, setIsLinking] = useState(false);
  const [isUnlinking, setIsUnlinking] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [successMessage, setSuccessMessage] = useState<string | null>(null);

  const fetchLineStatus = useCallback(async () => {
    setIsLoading(true);
    setError(null);

    try {
      const status = await usersApi.getLineStatus();
      setLineStatus(status);
    } catch (err) {
      setError('LINE連携状態の取得に失敗しました');
    } finally {
      setIsLoading(false);
    }
  }, []);

  useEffect(() => {
    fetchLineStatus();
  }, [fetchLineStatus]);

  useEffect(() => {
    if (successMessage) {
      const timer = setTimeout(() => setSuccessMessage(null), 3000);
      return () => clearTimeout(timer);
    }
  }, [successMessage]);

  const handleLinkLine = async () => {
    setIsLinking(true);
    setError(null);

    try {
      // LIFF SDKを初期化
      await initializeLiff();

      // LINEプロフィールを取得
      const profile = await getLiffProfile();

      // サーバーに連携リクエスト
      await usersApi.linkLine({
        lineUserId: profile.userId,
        displayName: profile.displayName,
        pictureUrl: profile.pictureUrl,
      });

      setLineStatus({
        isLinked: true,
        lineUserId: profile.userId,
        displayName: profile.displayName,
        pictureUrl: profile.pictureUrl,
      });

      setSuccessMessage('LINE連携が完了しました');
    } catch (err: any) {
      if (err.message?.includes('LIFF')) {
        setError('LINEアプリからアクセスしてください');
      } else {
        setError('LINE連携に失敗しました');
      }
    } finally {
      setIsLinking(false);
    }
  };

  const handleUnlinkLine = async () => {
    setIsUnlinking(true);
    setError(null);

    try {
      await usersApi.unlinkLine();

      setLineStatus({
        isLinked: false,
      });

      setSuccessMessage('LINE連携を解除しました');
    } catch (err) {
      setError('LINE連携の解除に失敗しました');
    } finally {
      setIsUnlinking(false);
    }
  };

  if (isLoading) {
    return <Loading message="読み込み中..." />;
  }

  if (error && !lineStatus) {
    return <Error message={error} onRetry={fetchLineStatus} />;
  }

  return (
    <div className="pb-20">
      <header className="bg-white shadow-sm p-4 mb-4">
        <div className="flex items-center">
          <button
            onClick={() => navigate(-1)}
            className="flex items-center text-gray-600 hover:text-gray-800 min-w-[44px] min-h-[44px]"
          >
            <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
            </svg>
            <span className="ml-1">戻る</span>
          </button>
        </div>
        <h1 className="text-xl font-bold text-gray-800 mt-2">LINE連携</h1>
      </header>

      {/* 成功メッセージ */}
      {successMessage && (
        <div className="mx-4 mb-4 p-3 bg-green-100 border border-green-300 text-green-700 rounded-lg">
          {successMessage}
        </div>
      )}

      {/* エラーメッセージ */}
      {error && (
        <div className="mx-4 mb-4 p-3 bg-red-100 border border-red-300 text-red-700 rounded-lg">
          {error}
        </div>
      )}

      <div className="px-4">
        {/* LINE連携状態表示 */}
        <section className="bg-white rounded-lg shadow p-6 mb-6">
          <div className="flex items-center mb-4">
            {/* LINE アイコン */}
            <div className="w-12 h-12 bg-line-green rounded-full flex items-center justify-center mr-4">
              <svg className="w-7 h-7 text-white" viewBox="0 0 24 24" fill="currentColor">
                <path d="M19.365 9.863c.349 0 .63.285.63.631 0 .345-.281.63-.63.63H17.61v1.125h1.755c.349 0 .63.283.63.63 0 .344-.281.629-.63.629h-2.386c-.345 0-.627-.285-.627-.629V8.108c0-.345.282-.63.627-.63h2.386c.349 0 .63.285.63.63 0 .349-.281.63-.63.63H17.61v1.125h1.755zm-3.855 3.016c0 .27-.174.51-.432.596-.064.021-.133.031-.199.031-.211 0-.391-.09-.51-.25l-2.443-3.317v2.94c0 .344-.279.629-.631.629-.346 0-.626-.285-.626-.629V8.108c0-.27.173-.51.43-.595.06-.023.136-.033.194-.033.195 0 .375.104.495.254l2.462 3.33V8.108c0-.345.282-.63.63-.63.345 0 .63.285.63.63v4.771zm-5.741 0c0 .344-.282.629-.631.629-.345 0-.627-.285-.627-.629V8.108c0-.345.282-.63.627-.63.349 0 .631.285.631.63v4.771zm-2.466.629H4.917c-.345 0-.63-.285-.63-.629V8.108c0-.345.285-.63.63-.63.349 0 .63.285.63.63v4.141h1.756c.348 0 .629.283.629.63 0 .344-.281.629-.629.629M24 10.314C24 4.943 18.615.572 12 .572S0 4.943 0 10.314c0 4.811 4.27 8.842 10.035 9.608.391.082.923.258 1.058.59.12.301.079.766.038 1.08l-.164 1.02c-.045.301-.24 1.186 1.049.645 1.291-.539 6.916-4.078 9.436-6.975C23.176 14.393 24 12.458 24 10.314" />
              </svg>
            </div>

            <div className="flex-1">
              <h2 className="text-lg font-semibold text-gray-800">
                LINE連携
              </h2>
              <p className={`text-sm ${lineStatus?.isLinked ? 'text-green-600' : 'text-gray-500'}`}>
                {lineStatus?.isLinked ? '連携済み' : '未連携'}
              </p>
            </div>
          </div>

          {lineStatus?.isLinked ? (
            /* 連携済み状態 */
            <div>
              <div className="flex items-center p-4 bg-gray-50 rounded-lg mb-4">
                {lineStatus.pictureUrl ? (
                  <img
                    src={lineStatus.pictureUrl}
                    alt="LINE プロフィール"
                    className="w-12 h-12 rounded-full mr-4"
                  />
                ) : (
                  <div className="w-12 h-12 bg-gray-300 rounded-full mr-4" />
                )}
                <div>
                  <p className="font-medium text-gray-800">
                    {lineStatus.displayName || 'LINEユーザー'}
                  </p>
                  <p className="text-sm text-gray-500">
                    {lineStatus.lineUserId}
                  </p>
                </div>
              </div>

              <p className="text-sm text-gray-600 mb-4">
                復習リマインダーがLINEに送信されます。
              </p>

              <button
                onClick={handleUnlinkLine}
                disabled={isUnlinking}
                className="w-full py-3 text-red-600 border border-red-600 rounded-lg hover:bg-red-50 min-h-[44px]"
              >
                {isUnlinking ? '解除中...' : '連携を解除'}
              </button>
            </div>
          ) : (
            /* 未連携状態 */
            <div>
              <p className="text-gray-600 mb-4">
                LINEと連携すると、復習リマインダーをLINEで受け取れます。
              </p>

              <ul className="text-sm text-gray-600 mb-6 space-y-2">
                <li className="flex items-start">
                  <svg className="w-5 h-5 text-green-500 mr-2 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                  </svg>
                  <span>毎日の復習リマインダーを受信</span>
                </li>
                <li className="flex items-start">
                  <svg className="w-5 h-5 text-green-500 mr-2 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                  </svg>
                  <span>LINEから直接アプリにアクセス</span>
                </li>
              </ul>

              <button
                onClick={handleLinkLine}
                disabled={isLinking}
                className={`w-full py-3 rounded-lg font-medium min-h-[44px] ${
                  isLinking
                    ? 'bg-gray-300 text-gray-500 cursor-not-allowed'
                    : 'bg-line-green text-white hover:bg-line-dark'
                }`}
              >
                {isLinking ? '連携中...' : 'LINEと連携する'}
              </button>
            </div>
          )}
        </section>

        {/* 注意事項 */}
        <section className="bg-gray-50 rounded-lg p-4">
          <h3 className="text-sm font-medium text-gray-700 mb-2">注意事項</h3>
          <ul className="text-sm text-gray-600 space-y-1">
            <li>- 連携にはLINEアプリが必要です</li>
            <li>- 連携を解除すると通知が届かなくなります</li>
            <li>- 連携情報は安全に保管されます</li>
          </ul>
        </section>
      </div>
    </div>
  );
};
```

**実装ファイル**: `frontend/src/pages/LinkLinePage.tsx`

### 2. LIFF SDK連携 🔵
**信頼性**: 🔵 *architecture.mdより*

LIFF SDKを使用したプロフィール取得を実装する。

```typescript
// frontend/src/services/liff.ts (追加)
import liff from '@line/liff';

export const initializeLiff = async (): Promise<void> => {
  if (!liff.isInClient()) {
    throw new Error('LIFF: LINEアプリ内からアクセスしてください');
  }

  await liff.init({ liffId: import.meta.env.VITE_LIFF_ID });

  if (!liff.isLoggedIn()) {
    liff.login();
    throw new Error('LIFF: ログインが必要です');
  }
};

export interface LiffProfile {
  userId: string;
  displayName: string;
  pictureUrl?: string;
  statusMessage?: string;
}

export const getLiffProfile = async (): Promise<LiffProfile> => {
  const profile = await liff.getProfile();
  return {
    userId: profile.userId,
    displayName: profile.displayName,
    pictureUrl: profile.pictureUrl,
    statusMessage: profile.statusMessage,
  };
};

export const isInLiffClient = (): boolean => {
  return liff.isInClient();
};
```

**実装ファイル**: `frontend/src/services/liff.ts`

### 3. API連携（LINE連携） 🔵
**信頼性**: 🔵 *architecture.mdより*

LINE連携状態の取得・連携・解除のAPI連携を実装する。

```typescript
// frontend/src/services/api.ts (部分)
interface LineStatus {
  isLinked: boolean;
  lineUserId?: string;
  displayName?: string;
  pictureUrl?: string;
}

interface LinkLineRequest {
  lineUserId: string;
  displayName: string;
  pictureUrl?: string;
}

export const usersApi = {
  // ... 他のメソッド

  getLineStatus: async (): Promise<LineStatus> => {
    const response = await apiClient.get<LineStatus>('/users/line-status');
    return response.data;
  },

  linkLine: async (data: LinkLineRequest): Promise<void> => {
    await apiClient.post('/users/link-line', data);
  },

  unlinkLine: async (): Promise<void> => {
    await apiClient.delete('/users/link-line');
  },
};
```

**実装ファイル**: `frontend/src/services/api.ts`

---

## 単体テスト要件

### テストケース1: 連携状態の表示（連携済み） 🔵
**信頼性**: 🔵 *user-stories.md 1.2より*
**Given**: LINEと連携済みのユーザー
**When**: LINE連携画面にアクセス
**Then**: 「連携済み」と連携先のLINEプロフィールが表示される

### テストケース2: 連携状態の表示（未連携） 🔵
**信頼性**: 🔵 *user-stories.md 1.2より*
**Given**: LINEと未連携のユーザー
**When**: LINE連携画面にアクセス
**Then**: 「未連携」と連携ボタンが表示される

### テストケース3: LINE連携の実行 🔵
**信頼性**: 🔵 *REQ-003より*
**Given**: 未連携状態で連携ボタンが表示されている
**When**: 「LINEと連携する」ボタンをクリック
**Then**: LIFF SDKでプロフィール取得後、サーバーに連携リクエストが送信される

### テストケース4: LINE連携成功時のメッセージ 🟡
**信頼性**: 🟡 *要件から妥当な推測*
**Given**: LINE連携処理を実行
**When**: 連携が成功
**Then**: 「LINE連携が完了しました」メッセージが表示される

### テストケース5: LINE連携の解除 🔵
**信頼性**: 🔵 *REQ-202より*
**Given**: 連携済み状態で解除ボタンが表示されている
**When**: 「連携を解除」ボタンをクリック
**Then**: サーバーに解除リクエストが送信され、未連携状態に戻る

### テストケース6: LIFF外からのアクセス時のエラー 🔵
**信頼性**: 🔵 *architecture.mdより*
**Given**: LINEアプリ外からアクセス
**When**: 連携ボタンをクリック
**Then**: 「LINEアプリからアクセスしてください」エラーが表示される

### テストケース7: 連携エラー時の表示 🟡
**信頼性**: 🟡 *要件から妥当な推測*
**Given**: LINE連携処理を実行中
**When**: エラーが発生
**Then**: エラーメッセージが表示される

---

## UI/UX要件

### ローディング状態 🔵
**信頼性**: 🔵 *NFR-201より*
- 状態読み込み中はローディングスピナーを表示
- 連携処理中はボタンに「連携中...」を表示

### 連携状態表示 🔵
**信頼性**: 🔵 *user-stories.md 1.2より*
- 連携済み: 緑色の「連携済み」ラベル + プロフィール情報
- 未連携: グレーの「未連携」ラベル + 連携ボタン

### エラー表示 🟡
**信頼性**: 🟡 *要件から妥当な推測*
- API取得エラー時はエラーメッセージを表示
- LIFF外アクセス時は専用のエラーメッセージを表示

### 連携成功メッセージ 🟡
**信頼性**: 🟡 *要件から妥当な推測*
- 連携成功時は緑色のバナーを表示
- 3秒後に自動で非表示

### モバイル対応 🔵
**信頼性**: 🔵 *NFR-201より*
- LINEの緑色（#00B900）をブランドカラーとして使用
- ボタンは44px以上のタッチ領域
- LINE WebView最適化

---

## 実装手順

### TDDタスクの場合
1. `/tsumiki:tdd-requirements TASK-0019` - 詳細要件定義
2. `/tsumiki:tdd-testcases` - テストケース作成
3. `/tsumiki:tdd-red` - テスト実装（失敗）
4. `/tsumiki:tdd-green` - 最小実装
5. `/tsumiki:tdd-refactor` - リファクタリング
6. `/tsumiki:tdd-verify-complete` - 品質確認

---

## 注意事項
- LINE連携はLIFF内（LINE WebView）からのみ実行可能
- 開発環境ではLIFF Inspectorを使用してテスト
- プロフィール画像のURLは外部URLのため、CSP設定に注意
- 連携解除後も既存の通知履歴は残る

---

## 信頼性レベルサマリー
- 🔵 **青信号**: 12項目
- 🟡 **黄信号**: 4項目
- 🔴 **赤信号**: 0項目

**品質評価**: 高品質
