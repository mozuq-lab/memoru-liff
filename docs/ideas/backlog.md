...
| AI URL カード生成 | `feature/url-card-generation` 他 | URL 入力 → コンテンツ取得 → AI カード生成。カードタイプ（QA / 定義 / 穴埋め）、枚数（5-30）、難易度選択対応。ブラウザプロファイルによる認証ページ対応 |
| AI チューター | `feature/tutor` 他 | デッキ単位の対話学習。フリートーク / クイズ / 弱点克服の 3 モード。TutorPage + tutor_sessions テーブル + DynamoDB セッション管理 |
...
### 目的
PDF ドキュメントの内容を AI が要約し、学習カードを自動生成する。

> **Note**: URL からのカード生成は実装済み（`POST /cards/generate-from-url`）。
> カードタイプ（QA / 定義 / 穴埋め）、枚数（5-30）、難易度、ブラウザプロファイルによる認証ページ対応まで完了している。
> この項目は PDF 入力の追加に焦点を当てる。

### 現状
- テキスト入力からの AI カード生成は実装済み（`POST /cards/generate`）
- URL からの AI カード生成は実装済み（`POST /cards/generate-from-url`）
...
- カード候補を表示し、ユーザーが選択して保存（既存の URL 生成フローと同じ）
- 生成カード数・カードタイプ・難易度を指定可能（既存の GenerateOptions を再利用）

### UI イメージ
- 既存の GeneratePage に「PDF」タブを追加（現在は「テキスト」「URL」の 2 タブ）
- PDF タブ: ファイルアップロード + ページ範囲指定 → カード生成

### アーキテクチャ
1. フロントエンドが PDF を S3 にアップロード（Pre-signed URL 方式、画像対応と共通化可能）
2. Lambda が S3 から PDF を取得、テキスト抽出（`pypdf` ライブラリ）
3. 抽出テキストを AI に渡してカード生成（既存の `generate_cards_from_chunks()` を再利用）

### API エンドポイント（案）
- `POST /cards/generate-from-pdf`
  - リクエスト: `{ "s3_key": "...", "page_range": "1-10", "card_type": "qa", "target_count": 10, "difficulty": "medium" }`
  - レスポンス: 既存の generate-from-url レスポンスと同形式

### 前提条件
- S3 バケットが必要（カード画像対応と基盤を共用可能）

### 実装難易度
中 — PDF テキスト抽出が主な新規実装。既存の AI カード生成パイプライン（チャンク化 + generate_cards_from_chunks）を再利用できる。S3 基盤はカード画像対応と共用可能。
...
| 3 | AI PDF カード生成 | 中 | S3 基盤（カード画像対応と共用可能） | 2nd（並行可） |

### 推奨理由
1. **日別復習グラフ**はインフラ変更なし・既存コンポーネント流用で最もコスト効率が高い
2. **カード画像対応**は S3 インフラ構築が必要だが、PDF 生成と基盤を共有できる
3. **AI PDF カード生成**は S3 基盤が整えば、既存の URL 生成パイプラインを再利用して比較的容易に実装可能
