...
- 対象はローカル開発環境のみ。`template.yaml`（SAM）・CDK・本番デプロイ設定は変更しない（「最終スコープ決定」によりElasticMQ本体は見送ったため、`SQS_ENDPOINT_URL` の template.yaml 宣言も不要）。
...
| `backend/template.yaml` | — | **見送り**。「最終スコープ決定」により `SQS_ENDPOINT_URL` 宣言は追加しない |
...
**Note:** 元プランにあった `template.yaml` への `SQS_ENDPOINT_URL` 宣言追加は、上記「最終スコープ決定」により見送り。
...
```makefile
.PHONY: help install build validate deploy-dev deploy-prod local-db local-keycloak local-ollama local-ollama-pull local-ollama-stop local-ollama-logs local-mq local-mq-stop local-all local-all-stop local-api local-api-mq mq-list-queues test clean
```
