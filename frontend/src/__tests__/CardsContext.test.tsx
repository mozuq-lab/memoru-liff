...
describe('TASK-0091: CardsContext fetchCards/fetchDueCards deckId パラメータ', () => {
  ...
  describe('fetchCards(deckId) パラメータ伝搬', () => {
    it('TC-091-001: deck_id 指定時に fetchCards が deckId パラメータ付きで API を呼び出す', async () => {
      ...
      await waitFor(() => {
        expect(cardsApi.getCards).toHaveBeenCalledWith('deck-abc-123', { signal: undefined }); // 【確認内容】: getCards の引数に deckId が含まれること 🔵
    ...
    it('TC-091-002: deck_id 未指定で fetchCards を呼ぶと全カードを取得する', async () => {
      ...
      await waitFor(() => {
        expect(cardsApi.getCards).toHaveBeenCalledWith(undefined, { signal: undefined }); // 【確認内容】: 引数なしの呼び出しで全カード取得 🔵
  ...
  describe('fetchDueCards(deckId) パラメータ伝搬', () => {
    it('TC-091-003: deck_id 指定時に fetchDueCards が deckId パラメータ付きで API を呼び出す', async () => {
      ...
      await waitFor(() => {
        expect(cardsApi.getDueCards).toHaveBeenCalledWith(undefined, 'deck-abc-123', { signal: undefined }); // 【確認内容】: getDueCards の引数に deckId が含まれること 🔵
      });
    });
    ...
    it('TC-091-004: deck_id 未指定で fetchDueCards を呼ぶと全復習対象カードを取得する', async () => {
      ...
      // 【期待値確認】: 引数なしの呼び出しで既存動作が壊れないこと
      await waitFor(() => {
        expect(cardsApi.getDueCards).toHaveBeenCalledWith(undefined, undefined, { signal: undefined }); // 【確認内容】: 引数なしの呼び出しで全復習対象カード取得 🔵
      });
    });
  });
});
...
/**
 * F-3: 古いリクエストの結果が破棄されること（タブ/デッキ連続切替時の上書き防止）
 * 【テスト対象】: fetchCards/fetchDueCards の最新リクエスト ID 管理
 */
describe('F-3: CardsContext 古いリクエストの結果破棄', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(cardsApi.getDueCount).mockResolvedValue(0);
  });

  // 解決を外部から制御できる Deferred を作るヘルパー
  ...
  const cardWithFront = (id: string, front: string): Card => ({
    card_id: id,
    user_id: 'user1',
    front,
    back: 'A',
  ...
  it('古い fetchCards の結果が新しい結果を上書きしないこと', async () => {
    const first = createDeferred<Card[]>();
    const second = createDeferred<Card[]>();
    ...
    // 【連続呼び出し】: deck-old を取得開始 → 解決前に deck-new を取得開始
    let p1: Promise<void>;
    let p2: Promise<void>;
    act(() => {
      p1 = result.current.fetchCards('deck-old');
      p2 = result.current.fetchCards('deck-new');
    });

    // 【後発リクエストを先に解決】: deck-new の結果がセットされる
    await act(async () => {
      second.resolve([cardWithFront('new', 'NEW')]);
      await p2;
    });
    expect(result.current.cards).toEqual([cardWithFront('new', 'NEW')]);

    // 【先発リクエストを後から解決】: 古い結果は破棄され上書きされないこと
    await act(async () => {
      first.resolve([cardWithFront('old', 'OLD')]);
      await p1;
    });
    expect(result.current.cards).toEqual([cardWithFront('new', 'NEW')]);
  });

  it('AbortError は error state に入らないこと', async () => {
    vi.mocked(cardsApi.getCards).mockRejectedValue(
    ...
    await act(async () => {
      await result.current.fetchCards('deck-x');
    });

    // 【検証】: AbortError は error として表面化しない
